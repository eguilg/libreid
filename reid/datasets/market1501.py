    ...
    def download(self):
        ...
        def register(subdir, pattern=re.compile(r'([-\d]+)_c(\d)s(\d)_([-\d]+)')):
            ...
            for fpath in fpaths:
                ...
                pid, cam, sequence, frame_stamp = map(int, pattern.search(fname).groups())
                ...
                samples.append((pid, cam, sequence, frame_stamp, fpath))
                sequence_size = max(sequence_size, frame_stamp)

            for pid, cam, sequence, frame_stamp, fpath in samples:
                frame_id = sequence * sequence_size + frame_stamp
                pids.add(pid)
